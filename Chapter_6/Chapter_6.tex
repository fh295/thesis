...
However, not all contextual models reach this level of performance. I find the way in which wider context is represented in memory to be critical.
If memories are encoded from a small window around important words in the context, there is an optimal size for memory representations between single words and entire sentences, that depends on the class of word to be predicted. I have nicknamed this effect the \emph{Goldilocks Principle} after the well-known English fairytale~\citep{goldilocks}. % (single word windows for syntactic words like prepositions, and larger windows for semantic words like named entities are the best fit);
%the optimal size of that window depends on the class of word to be predicted (single word windows for syntactic words like prepositions, and larger windows for semantic words like named entities);
%
%they are more useful to the network than those encoding either single words or entire sentences;
%
...
% Wrong
% the encode the query $q$ using a feature map \(\phi_q(q)\) mapping sequences of words to one-hot representations \([0,1]^d\), with $d$ the size of the word % vocabulary. As a result, queries are encoded using bag-of-words representations.
%Among the \(D\)-dimensional feature space I include \(|W|\) query-specific features for each word in the vocabulary \(W\). 
% For each \(w \in q\) the corresponding feature takes the value \(T_{c}(w,q)\), where \(c\) is the entire training corpus and \(T_{D}(x,t)\) is the \emph{tfidf} of \(x\) in term \(t\) within document \(D\).    
...
%, resulting in two features per memory slot. % (see below). 
...
%I found \(b=5\) to work best in practice. 
%
%The query is also replaced by a window of size $b$ centred around the missing word $q_m$.
...
% train the scoring function to retrieve a memory conditioned on both the query and an already selected memory, most simply with \( S_{o}( [q,\textbf{m}_{o1}], \textbf{m}_i) = S_{o}(q, \textbf{m}_i) + S_{o}(\textbf{m}_{o1}, \textbf{m}_i) \). I can compute a second softmax weighting based on \(S_o\) and \(\textbf{m}_{o1}\), and thus a second memory \(\textbf{m}_{o2}\) based on the first. This repeated memory access (referred to as `hops' by \cite{sukhbaatar2015end}) can be iterated to yield \(k\) output memories for each question. In practice, I noticed marginal improvement for large \(k\), and report results with \(k = 3\).
...
%I did not train the model to discern candidates in the question directly, e.g. with a ranking loss, and this approach is left as future work.
%
...
\section{Baseline and Comparison Models}
